...
import { GroupContext } from "../context"; // Import the context
...
export default function SideBar() {
  ...
  const menuItems = [
    { name: "Home", path: "/home" },
    { name: hasJoinedTeam ? "Work Space" : "My Groups", path: hasJoinedTeam ? "/work-space" : "/my-groups" },
        ...
        {menuItems.map((item) => (
          <li
            ...
            className={`w-[90%] text-center p-3 border-b cursor-pointer transition duration-300 transform 
              ${selectedPath === item.path ? "text-[#00A99B] border-[#00A99B] border-b-2 font-semibold w-full" : "border-[#B2DEDA]"}
